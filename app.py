...
# --- 1. محرك قاعدة البيانات المطور (دعم المشاريع والمستخدمين) ---
class ProgressTrackerDB:
    ...
    def add_project(self, name, client):
        try:
            cursor = self.conn.cursor()
            cursor.execute("INSERT INTO projects (project_name, client_name, created_at) VALUES (?, ?, ?)",
    ...
    def record_wir(self, project_id, wir_id, date, boq_code, location, approved_qty, status, user_name):
        summary = self.get_boq_summary(project_id, boq_code)
        if summary:
            remaining_qty = summary['remaining_qty']
            if approved_qty > remaining_qty:
                return False, f"خطأ: الكمية المدخلة ({approved_qty}) تتجاوز المتبقي ({remaining_qty})!"
        
        cursor = self.conn.cursor()
        cursor.execute('''
            INSERT INTO wir_log (project_id, wir_id, wir_date, boq_code, location, approved_qty, status, created_by)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (project_id, wir_id, date, boq_code, location, approved_qty, status, user_name))
        self.conn.commit()
    ...
    def get_boq_summary(self, project_id, boq_code=None):
        ...
        if boq_code:
            ...
            df = pd.read_sql_query(query, self.conn, params=(project_id, boq_code))
...
st.set_page_config(page_title="شركة العامرية المتحدة للمقاولات - إدارة المشاريع", layout="wide", page_icon="🏗️")

st.title("🏗️ منصة إدارة المشاريع والكميات - شركة العامرية المتحدة للمقاولات")

# --- الشريط الجانبي: بيانات المهندس والتواصل والمشروع ---
...
user_name = st.sidebar.text_input("اسم المستخدم / المهندس:", value="م. أحمد السيد")

projects_df = db.get_projects()

if not projects_df.empty:
    project_options = dict(zip(projects_df['project_name'], projects_df['project_id']))
    selected_project_name = st.sidebar.selectbox("📂 اختر المشروع الحالي:", list(project_options.keys()))
    ...
    if not boq_df.empty:
        ...
        approved_qty = st.sidebar.number_input("الكمية المعتمدة:", min_value=0.0, step=1.0)
        status = st.sidebar.selectbox("حالة الاعتماد:", ["Code A", "Code B"])

        if st.sidebar.button("حفظ الـ WIR"):
            success, msg = db.record_wir(selected_project_id, wir_no, str(wir_date), selected_boq, location, approved_qty, status, user_name)
            if success:
    ...
    st.sidebar.warning("لا توجد مشاريع مضافة حالياً. يرجى إضافة مشروع جديد.")
    selected_project_id = None

# --- تبويبات المنصة ---
tab1, tab2, tab3 = st.tabs(["📊 تقرير المشروع الحسابي", "⚙️ إعدادات BOQ المشروع", "➕ إدارة المشاريع"])

# Tab 1: تقارير المشروع المختار
with tab1:
    if selected_project_id:
        st.subheader(f"📊 تقرير البروجريس الخاص بمشروع: ({selected_project_name})")
        ...
        if not summary_df.empty:
            ...
            overall_progress = (total_earned / total_contract * 100) if total_contract > 0 else 0
            ...
            st.download_button(f"📥 تصدير تقرير ({selected_project_name})", excel_data, f"{selected_project_name}_Progress.csv", "text/csv")
        else:
            st.info("لا توجد بنود تعاقدية مضافة لهذا المشروع بعد.")
    else:
        st.warning("يرجى اختيار أو إضافة مشروع من القائمة الجانبية أو تبويب إدارة المشاريع.")

# Tab 2: إعدادات BOQ الخاص بالمشروع المختار
with tab2:
    if selected_project_id:
        st.subheader(f"إضافة بنود BOQ لمشروع: ({selected_project_name})")
        with st.form("boq_form"):
            ...
            if st.form_submit_button("حفظ البند في هذا المشروع"):
                db.add_boq_item(selected_project_id, b_code, b_desc, b_unit, b_qty, b_rate)
                st.success("تم حفظ البند بنجاح داخل هذا المشروع!")
                st.rerun()
    else:
        st.warning("قم باختيار أو إنشاء مشروع أولاً لكي تتمكن من إدخال البنود.")

# Tab 3: إنشاء وإدارة المشاريع الجديدة
with tab3:
...
st.divider()
st.markdown("<center>© جميع الحقوق محفوظة - تطوير م. أحمد السيد | شركة العامرية المتحدة للمقاولات</center>", unsafe_allow_html=True)